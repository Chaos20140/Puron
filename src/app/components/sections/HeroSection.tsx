  ...
  return (
    <header className="relative pt-28 pb-16 md:pt-48 md:pb-24">
      <div className="max-w-7xl mx-auto px-5 md:px-6 relative z-10 grid lg:grid-cols-2 gap-12 items-center">
        {/* NO `opacity: 0` start here — this block holds the <h1>, which is the
            LCP element on phones. An opacity-gated entrance makes the browser
            treat the heading as not-yet-painted until the animation has run,
            which pushed LCP far behind FCP. The slide-up is transform-only:
            it reads the same but the text counts as painted immediately. */}
        <motion.div
          initial={{ y: 24 }}
          animate={{ y: 0 }}
