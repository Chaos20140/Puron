...
- `ALLOWED_ORIGINS` (optional, default `*`) — comma-separated allow-list. Production is now live on `https://puron-media.de`, so this should be set to `https://puron-media.de` (add `,https://www.puron-media.de` if a www record is configured) to stop other sites POSTing to `/contact`. Still default `*` as of 2026-06-02 — outstanding hardening item.
