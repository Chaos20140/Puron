...
- `CONTACT_EMAIL_FROM` (optional, default `onboarding@resend.dev`). **Set this to a sender on a Resend-verified domain (e.g. `noreply@puron-media.de`) to lift the test-mode recipient restriction above AND keep mail out of spam.** Verify the domain at resend.com/domains by adding the DKIM/SPF/return-path DNS records. SPF caveat: puron-media.de already has an M365 SPF (`include:spf.protection.outlook.com`) — **merge** Resend's `include:` into that single TXT record, don't add a second SPF record. The M365 apex MX (for *receiving* at info@) is untouched by Resend's records (return-path MX sits on a `send.` subdomain).
