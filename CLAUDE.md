...
- `CONTACT_EMAIL_FROM` — **now SET (live, 2026-06-24) to `noreply@puron-media.de`** (was `onboarding@resend.dev`). This lifts the Resend test-mode recipient restriction AND keeps mail out of spam — but only **once `puron-media.de` is verified** at resend.com/domains. Add the records Resend shows: DKIM TXT on `resend._domainkey`, SPF TXT + return-path MX on the `send.` subdomain, optional DMARC on `_dmarc`. These all sit on **subdomains**, so they do **not** conflict with the apex M365 SPF/MX (no SPF merge needed with the modern `send.`-subdomain setup) or the GitHub-Pages A records. Once Resend shows "Verified", the form works with no further change (secret already set).
