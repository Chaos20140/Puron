...
> **Contact page state (2026-06-23):** [ContactPage.tsx](src/app/components/pages/ContactPage.tsx) has the **email contact form back** (restored from `2e5d5cf^`: name/company/email/message + goal chips, honeypot, client validation, success state — posts to `POST /contact`), plus the direct-channel cards (E-Mail `info@puron-media.de`, WhatsApp, Instagram `@puronmedia`, Telefon) kept as a fallback row below it. The **Google-Maps embed was removed** (so the CSP `frame-src` is back to `'none'` in both [vite.config.ts](vite.config.ts) and [public/_headers](public/_headers), and the Datenschutz Google-Maps clause is gone — [PrivacyPage.tsx](src/app/components/pages/PrivacyPage.tsx)). WhatsApp is also a **persistent floating FAB on every page** ([WhatsAppButton.tsx](src/app/components/WhatsAppButton.tsx), mounted in Layout); the wa.me link lives in [src/app/whatsapp.ts](src/app/whatsapp.ts). **Mail delivery — the form WORKS as of 2026-06-24**, via Resend in **test-mode** (the "quick path" the user chose). Why not the nicer options: Microsoft Graph is coded + deployed but needs `Mail.Send` admin consent from a Global Admin the user isn't (greyed-out "Grant admin consent"); verifying the Resend domain (DNS) was deemed too much for now. Resend test-mode (sender `onboarding@resend.dev`) only delivers to the **Resend account's own signup address**, which Resend confirmed is **`tolunayusul@gmail.com`** — so the live secrets are `CONTACT_EMAIL_FROM=onboarding@resend.dev` + `CONTACT_EMAIL_TO=tolunayusul@gmail.com`. Verified live (HTTP 200, message id) on 2026-06-24. **Trade-off:** inquiries land in that personal Gmail, **not** `info@`. **Path to `info@` (decided 2026-06-24):** the user has **mailbox access but NO DNS/domain access** (and isn't M365 admin), so Resend-domain-verify (needs DNS) and MS Graph (needs admin consent) are both out. The route is **Web3Forms** — a form-to-email service verified via the *inbox*, not the domain. The handler now relays through Web3Forms when `WEB3FORMS_ACCESS_KEY` is set (top-priority provider, [index.ts](supabase/functions/make-server-1fdc8e05/index.ts) `web3Key` branch — code shipped + deployed, dormant until the key exists). Activation: create a key at web3forms.com with `info@puron-media.de` (key emailed to that inbox) → `supabase secrets set WEB3FORMS_ACCESS_KEY=… --project-ref …` → **redeploy** (warm isolates cache env) → `/contact` delivers to `info@`. Then set `CONTACT_EMAIL_TO` back to `info@puron-media.de` (cosmetic; Web3Forms routes by the key). Note the per-IP rate limit is 3/hour — repeated testing from one IP returns 429.
